...
8. 回答的字数控制在300字以内，格式采用 Markdown，条理清晰：
   - 用 **加粗** 标出关键概念或结论；
   - 用 ## 小标题 分段（如「直观解释」「严格结论」「总结」），每段下用短段落或列表；
   - 多要点时用 - 无序列表或 1. 2. 有序列表；
   - 段落之间空一行，避免大段不分段。
9. 数学公式必须用美元符号包裹，以便前端正确渲染：
   - 行内公式：用一对 $ 包裹，例如 $\\lim_{x \\to a} f(x)=f(a)$、$f'(a)=\\lim_{h \\to 0}\\frac{f(a+h)-f(a)}{h}$；
   - 独立一行公式：用一对 $$ 包裹，例如 $$\\frac{\\partial f}{\\partial x_i}(a)=\\lim_{h \\to 0}\\frac{f(a+h e_i)-f(a)}{h}$$；
   - 使用 LaTeX 语法（\\frac、\\lim、\\partial、\\sum、\\to 等），不要写 lim_{x->a} 这种纯文本，要写 $\\lim_{x \\to a}$。
"""
